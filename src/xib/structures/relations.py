...
class VisualRelations(object):
    ...
    def predicate_str(self) -> Sequence[str]:
        if self.predicate_classes is not None:
            if self.predicate_vocabulary is not None:
                predicates = self.predicate_vocabulary.get_str(
                    self.predicate_classes.detach().cpu()
                ).tolist()
            else:
    ...
    def subject_str(self) -> Sequence[str]:
        if self.instances is not None:
            return instance_str(
                self.instances[self.relation_indexes[0]], self.object_vocabulary
            )
        else:
    ...
    def object_str(self) -> Sequence[str]:
        if self.instances is not None:
            return instance_str(
                self.instances[self.relation_indexes[1]], self.object_vocabulary
            )
        else:
    ...
    def relation_str(self):
        ...
        return [
            ...
            for s, p, o, score in zip(
                self.subject_str(), self.predicate_str(), self.object_str(), scores
            )
        ]
    ...
    def cuda(
        self, device=Union[str, torch.device], non_blocking=False
    ) -> VisualRelations:
        return self.to(device, non_blocking)

    def to(
        self, device=Union[str, torch.device], non_blocking=False, copy=False
    ) -> VisualRelations:
    ...
    def detach(self) -> VisualRelations:
        return dataclasses.replace(
            self, **{k: v.detach() for k, v in self._iter_fields()}
        )
    ...
    def __getitem__(self, item: Union[int, slice, torch.BoolTensor]) -> VisualRelations:
        return dataclasses.replace(self, **{k: v[item] for k, v in self._iter_fields()})

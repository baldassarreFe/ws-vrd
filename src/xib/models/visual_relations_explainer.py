    ...
    def _explain_visual_relations(self, inputs: Batch) -> Dict[str, Batch]:
        ...
        with torch.enable_grad():
            ...
            for k, k_th_predicate_score in enumerate(
                ...
                relevance_nodes[:, k] = inputs.object_linear_features.grad.norm(
                    dim=1, p=1
                ) + inputs.object_conv_features.grad.flatten(start_dim=1).norm(
                    dim=1, p=1
                )
                relevance_edges[:, k] = inputs.relation_linear_features.grad.norm(
                    dim=1, p=1
                )
        ...
        # - the confidence score given to the subject from the object detector
        # - the confidence score given to the object from the object detector
        ...
        relations_with_obj_scores = self._keep_top_x_relations(
            B,
        ...
        relations_without_obj_scores = self._keep_top_x_relations(
            B,
            edge_to_graph_assignment,
            inputs,
            predicate_classes_sorted,
        ...
        return {
            "with_obj_scores": relations_with_obj_scores,
            "no_obj_scores": relations_without_obj_scores,
        }
